...
async def upload_file(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if file.content_type != 'application/pdf':
        return JSONResponse({"status": "error", "message": "Unsupported file type. Please upload a PDF file."}, status_code=400)
    ...
    try:
        ...
        if extraction_result is None:
            return JSONResponse({"status": "error", "message": "Error extracting text and metadata from PDF."}, status_code=500)
        ...
        return JSONResponse({"status": "error", "message": "Error uploading file."}, status_code=500)
...
async def ask(request: QueryRequest):
    ...
    if not os.listdir(folder):
        ...
        return JSONResponse({"status": "error", "message": "No files found in the uploaded files directory."}, status_code=400)

    # Checking if the NLP model (query_engine) is initialized
    if query_engine is None:
        print("NLP model has not been initialized")
        return JSONResponse({"status": "error", "message": "The NLP model has not been initialized."}, status_code=500)
    ...
    try:
        ...
        if answer == "Empty Response":
            ...
            return JSONResponse({"status": "success", "message": "Sorry, I couldn't understand your question. Could you please rephrase it?"})

        print(f"Query processed successfully: {answer}")
        return JSONResponse({"status": "success", "data": {"answer": answer}})
    except Exception as e:
